define('app/exts/auth/lib/rule/default',['require','module','exports','jquery','underscore'],function(require,module,exports){  var $ = require('jquery')
  ...
  return {
    ...
    required: function(value, attr, defer, field) {
      if(!this.msg('error')) this.msg('error', '此项为必填项！')
      var $target = this.get('target')
      var groupEls = ['radio', 'checkbox']
      if ($.inArray($target.attr('type'), groupEls) != -1) {
        var checked = false
    ...
    mobile: function(value) {
      ...
      if (value == '') return true
      return /^0?\d{9,11}$/.test($.trim(value))
    },
    /**
     * 是否符合日期格式
