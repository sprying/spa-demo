/*!Magix 2.0 Licensed MIT*/
(function (NULL, WINDOW, DOCUMENT, NOOP, SPLITER, EMPTY, COMMA, LIB, IdIt, COUNTER) {
  COUNTER = 1;
  IdIt = function (n) {
    return n.id || (n.id = 'mx_n_' + COUNTER++)
  };
  /**
  ...
  LIB('magix', ['jquery', 'brix/event'], function ($, BXEvent) {
     ...
     * };
       *
       * var result=Magix.toTry([f1,f2],new Date().getTime());
       *
     ...
     * });
      ...
      start: function (cfg) {
        var me = this;
        Mix(Cfg, cfg);
        G_Require([cfg.iniFile], function (I) {
          Cfg = Mix(Cfg, I, cfg);
          Cfg['!tnc'] = Cfg.tagName != DefaultTagName;

          Router.on('!ul', VOM.loc);
          Router.on('changed', VOM.loc);

          G_Require(Cfg.extensions || Cfg.exts, Router.start);
        });
      },
      /**
       * 获取对象的keys
    ...
    Mix(Mix(Vframe.prototype, Event), {
      ...
      mountView: function (viewPath, viewInitParams, keepPreHTML) {
        ...
        if (viewPath) {
          ...
          var vn = po.path;
          var sign = ++me.sign;
          G_Require(vn, function (TView) {
            if (sign == me.sign) { //有可能在view载入后，vframe已经卸载了

              View.prepare(TView, RefVOM);
      ...
      invokeView: function (name, args) {
        ...
        if (vf.viewInited) {
          ...
          if (fn) {
            result = ToTry(fn, args || EmptyArr, view);
          }
        }
        return result;
      }
 ...
 *  }
    ...
    View.prepare = function (oView) {
      if (!oView[SPLITER]) { //只处理一次
        ...
        prop.render = WrapFn(prop.render);
      }
    };
 ...
 *  });
     ...
     * }
       ...
       * @param {Boolean} [preventWhenRenderCall] 当render方法被调用时，是否阻止回调的调用
       * @return {Function}
       * @example
       * render:function(){
     ...
     * }
       ...
       * //
       */
      wrapAsync: function (fn, preventWhenRenderCall) {
        var me = this;
        var sign = me.sign;
        return function () {
          if (sign > 0 && (!preventWhenRenderCall || sign == me.sign)) {
